...
-- SELECT 쿼리에 함수나 산술 연산이 기술되어 있는 경우 별칭을 지정해야 한다.
SELECT emp_name, 
...
VALUES('300', '홍길동', '250701-3123456', 30000000);
...
-- 5. WITH CHECK OPTION
-- 서버 쿼리에 기술된 조건에 부합하지 않는 값으로 수정하는 경우 오류를 발생시킨다.
CREATE OR REPLACE VIEW v_employee
AS SELECT *
	FROM employee
...
-- 선동일 사원의 급여를 2000000원으로 변경
-- 서브 쿼리의 조건에 부합하지 않기 때문에 변경이 불가능하다.
UPDATE v_employee
SET salary = 2000000
WHERE emp_id = '200';

-- 선동일 사원의 급여를 4000000원으로 변경
